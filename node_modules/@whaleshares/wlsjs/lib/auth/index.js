...
/**
	...
	@arg {array} roles - defaults to standard Whaleshares blockchain-level roles
*/
...
Auth.reqWhaleVaultSig = function (trx, keys, keyType) {
	if (config.get('whalevault') != null) {
		...
		return config.get('whalevault').promiseRequestSignBuffer('wlsjs', 'wls:' + user_arr[0], wv_ops, keyType, user_arr[1], 'tx').then(function (response) {
			if (response.success) {
...
Auth.signTransaction = function (trx, keys) {
	...
	if (config.get('whalevault') != null && keys[keyType].includes(':')) {
		return this.reqWhaleVaultSig(trx, keys, keyType).then(function (trxFinal) {
