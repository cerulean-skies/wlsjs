...
var debug = (0, _debug2.default)('wlsjs:broadcast');
...
/**
 * Sign and broadcast transactions on the Whaleshares network
 */

wlsBroadcast.send = function wlsBroadcast$send(tx, privKeys, callback) {
  var resultP = wlsBroadcast._prepareTransaction(tx).then(function (transaction) {
...
wlsBroadcast._prepareTransaction = function wlsBroadcast$_prepareTransaction(tx) {
...
_operations2.default.forEach(function (operation) {
  ...
  wlsBroadcast[operationName + 'With'] = function wlsBroadcast$specializedSendWith(wif, options, callback) {
    debug('Sending operation "' + operationName + '" with', { options: options, callback: callback });
    var keys = {};
    if (operation.roles && operation.roles.length) {
      keys[operation.roles[0]] = wif; // TODO - Automatically pick a role? Send all?
    }
    return wlsBroadcast.send({
      extensions: [],
  ...
  wlsBroadcast[operationName] = function wlsBroadcast$specializedSend(wif) {
    ...
    return wlsBroadcast[operationName + 'With'](wif, options, callback);
  };
});
...
(0, _helpers2.default)(wlsBroadcast);

_bluebird2.default.promisifyAll(wlsBroadcast);

exports = module.exports = wlsBroadcast;